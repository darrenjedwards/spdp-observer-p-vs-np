...
Computes the SPDP rank for a polynomial with overlapping supports:
    f(x) = x0*x1 + x1*x2 + x2*x3

Tests how SPDP responds to partial entanglement across variable chains.
...
# Define overlapping chain polynomial
x0, x1, x2, x3 = sp.symbols('x0 x1 x2 x3')
chain_expr = sp.expand(x0*x1 + x1*x2 + x2*x3)
...
# Run test
compute_spdp(chain_expr, "Overlapping Support Chain")
