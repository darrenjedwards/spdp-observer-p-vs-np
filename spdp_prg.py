...
Computes the SPDP rank for a pseudorandom generator-style polynomial:
    f(x) = x0*x1 + x2*x3 + x4*x5

This structure mimics Goldreich-style local PRGs (k-local predicates).
...
# Define PRG-style function (3 local predicates)
x0, x1, x2, x3, x4, x5 = sp.symbols('x0 x1 x2 x3 x4 x5')
prg_expr = sp.expand(x0 * x1 + x2 * x3 + x4 * x5)
...
# Run test
compute_spdp(prg_expr, "Goldreich PRG-style Polynomial")
