...
Computes the SPDP rank for the 3-variable XOR function and a noisy variant
using k-th order directional derivatives and low-degree shift monomials.

SPDP parameters:
    k = 2   # order of partial derivatives
    ℓ = 1   # degree of shift monomials
...
# Define XOR (real-valued algebraic normal form) and noisy XOR
x0, x1, x2 = sp.symbols('x0 x1 x2')
xor_expr = sp.expand(x0 + x1 + x2 - 2*(x0*x1 + x0*x2 + x1*x2) + 4*x0*x1*x2)
noisy_xor_expr = sp.expand(xor_expr + 0.01 * x0 * x1 * x2)
...
# Run tests
compute_spdp(xor_expr, "XOR")
compute_spdp(noisy_xor_expr, "Noisy XOR")
