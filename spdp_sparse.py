...
Computes the SPDP rank for a sparse high-degree polynomial:
    f(x) = x0^5 + x1^5 + x2^5

This tests SPDP's ability to detect structural triviality even when degree is high.
SPDP should collapse such separable, unentangled functions.
...
# Define sparse high-degree polynomial
x0, x1, x2 = sp.symbols('x0 x1 x2')
sparse_expr = sp.expand(x0**5 + x1**5 + x2**5)
vars_list = [x0, x1, x2]
...
# Run test
compute_spdp(sparse_expr, "High-degree Sparse")
